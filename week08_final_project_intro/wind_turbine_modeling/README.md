...
6. Find operational strategy, i.e.,  blade pitch angle ($\theta_p$)
...
* Pay attention to the polar files, they don't necessarily follow the exact same
format. Be clear about which data you will need to get out of each file.
* When you are done with the listed functional requirements, consider extending
 your package with more functionalites, for example:
    ...
    * You may compare the power curve and thrust curve you obtained with the
    power curve and thrust curve provided in `IEA_15MW_RWT_Onshore.opt`.
    ...
    * You may consider adding an optimization tool to find the optimal 
    operational strategy , i.e.,  blade pitch angle ($\theta_p$)
    and rotational speed ($\omega$), as function of wind speed ($V_0$), and 
    compare it with the strategy provided in `IEA_15MW_RWT_Onshore.opt`.
